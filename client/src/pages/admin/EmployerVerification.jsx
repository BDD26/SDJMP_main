  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <Badge className="mb-4 bg-primary/10 text-primary hover:bg-primary/20 transition-colors py-1 px-3 border-none capitalize">
            Employer Moderation
          </Badge>
          <h1 className="text-5xl font-black tracking-tight font-heading">Verification <span className="text-primary">Queue</span></h1>
          <p className="text-muted-foreground text-lg mt-3 max-w-xl">
            Review and verify employer accounts to maintain a secure and trustworthy marketplace.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-none shadow-xl glass p-6">
          <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Pending Approval</p>
          <h4 className="text-3xl font-black">{employers.filter(e => e.status === 'pending').length}</h4>
        </Card>
        <Card className="border-none shadow-xl glass p-6">
          <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Total Verified</p>
          <h4 className="text-3xl font-black text-emerald-500">{employers.filter(e => e.status === 'verified').length}</h4>
        </Card>
        <Card className="border-none shadow-xl glass p-6">
          <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Flagged/Suspended</p>
          <h4 className="text-3xl font-black text-rose-500">{employers.filter(e => e.status === 'suspended').length}</h4>
        </Card>
      </div>

      <div className="grid gap-6">
        {employers.map((employer) => (
          <Card key={employer.id} className={`border-none shadow-xl glass hover:shadow-2xl transition-all duration-300 group overflow-hidden relative ${employer.status === 'suspended' ? 'opacity-60 grayscale' : ''}`}>
